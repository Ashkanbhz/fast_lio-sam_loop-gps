...
void publish_frame_world(const ros::Publisher & pubLaserCloudFull)
{
    if(scan_pub_en) // 默认发布
    {
        ...
        laserCloudmsg.header.frame_id = odometry_frame;
...
void publish_effect_world(const ros::Publisher & pubLaserCloudEffect)
{
    ...
    laserCloudFullRes3.header.frame_id = odometry_frame;
...
void publish_map(const ros::Publisher & pubLaserCloudMap)
{
    ...
    laserCloudMap.header.frame_id = odometry_frame;
    pubLaserCloudMap.publish(laserCloudMap);
}

// 设置输出的t,q，在publish_odometry，publish_odom_path调用
template<typename T>
...
{
    odomAftMapped.header.frame_id = odometry_frame;
    ...
    transform.setRotation( q );
    br.sendTransform( tf::StampedTransform( transform, odomAftMapped.header.stamp, odometry_frame, "imu_link" ) );
...
void publish_odom_path(const ros::Publisher pubPath)
{
    set_posestamp(msg_body_pose);
    msg_body_pose.header.stamp = ros::Time().fromSec(lidar_end_time);
    msg_body_pose.header.frame_id = odometry_frame;
